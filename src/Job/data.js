export const jobsData = [
  {
    company: 'Uber Technologies Inc',
    title: 'Sr. Product Designer',
    tags: ['User Interface', 'User Research / Flow'],
    logo: require('../../assets/job/uber.png'),
    hoursAgo: '50 min ago',
    category: 'Design',
    location: '1455 Market St #400, San Francisco',
    totalApplied: 10,
    applied: [
      {
        img: require('../../assets/job/image1.png'),
      },
      {
        img: require('../../assets/job/image2.png'),
      },
      {
        img: require('../../assets/job/image3.png'),
      },
      {
        img: require('../../assets/job/image4.png'),
      },
      {
        img: require('../../assets/job/image2.png'),
      },
    ],
    order: {
      message: 'Be in the first',
      countMsg: '10 applicants',
    },
    details: {
      description:
        "We're a team of youthful, intelligent and dedicated creative who have an unrivaled energy and passion for crafting beautiful and meaningful products\n\nWe strive to follow all UX processes of ideation research and MVP for our product designs to enable output that is both aesthetic and strategic",
      responsibilities: 'Keep the interface beautiful and easy to use',
      jobType: 'Full-Time',
      salary: '$80,000 - $140,000 a year',
      NumOfHires: 2,
    },
  },
  {
    company: 'Cisco',
    title: 'Network Engineer',
    tags: ['CCT, CCENT, Net+', 'Management'],
    logo: require('../../assets/job/cisco.jpeg'),
    hoursAgo: '1 hour ago',
    category: 'Network',
    location: '1000 W Maude Ave, Sunnyvale, CA',
    totalApplied: 24,
    applied: [
      {
        img: require('../../assets/job/image4.png'),
      },
      {
        img: require('../../assets/job/image2.png'),
      },
      {
        img: require('../../assets/job/image3.png'),
      },
    ],
    order: {
      message: 'Be in the first',
      countMsg: '24 applicants',
    },
    details: {
      description:
        "We're a team of youthful, intelligent and dedicated creative who have an unrivaled energy and passion for crafting beautiful and meaningful products\n\nWe strive to follow all UX processes of ideation research and MVP for our product designs to enable output that is both aesthetic and strategic",
      responsibilities: 'Keep the interface beautiful and easy to use',
      jobType: 'Full-Time',
      salary: '$90,000 - $150,000 a year',
      NumOfHires: 2,
    },
  },
  {
    company: 'Google',
    title: 'Cybersecurity Analyst',
    tags: ['Microsoft O365 suite, Azure', 'SSCP'],
    logo: require('../../assets/job/youtube.jpg'),
    hoursAgo: '3 hours ago',
    category: 'Security',
    location: '901 Cherry Ave, San Bruno',
    totalApplied: 13,
    applied: [
      {
        img: require('../../assets/job/image1.png'),
      },
      {
        img: require('../../assets/job/image2.png'),
      },
      {
        img: require('../../assets/job/image3.png'),
      },
      {
        img: require('../../assets/job/image4.png'),
      },
      {
        img: require('../../assets/job/image2.png'),
      },
    ],
    order: {
      message: 'Be in the first',
      countMsg: '13 applicants',
    },
    details: {
      description:
        "We're a team of youthful, intelligent and dedicated creative who have an unrivaled energy and passion for crafting beautiful and meaningful products\n\nWe strive to follow all UX processes of ideation research and MVP for our product designs to enable output that is both aesthetic and strategic",
      responsibilities: 'Keep the interface beautiful and easy to use',
      jobType: 'Full-Time',
      salary: '$90,000 - $150,000 a year',
      NumOfHires: 2,
    },
  },
  {
    company: 'Uber Technologies Inc',
    title: 'Sr. Product Designer',
    tags: ['User Interface', 'User Research / Flow'],
    logo: require('../../assets/job/uber.png'),
    hoursAgo: '50 min ago',
    category: 'Design',
    location: '1455 Market St #400, San Francisco',
    totalApplied: 10,
    applied: [
      {
        img: require('../../assets/job/image1.png'),
      },
      {
        img: require('../../assets/job/image2.png'),
      },
      {
        img: require('../../assets/job/image3.png'),
      },
      {
        img: require('../../assets/job/image4.png'),
      },
      {
        img: require('../../assets/job/image2.png'),
      },
    ],
    order: {
      message: 'Be in the first',
      countMsg: '10 applicants',
    },
    details: {
      description:
        "We're a team of youthful, intelligent and dedicated creative who have an unrivaled energy and passion for crafting beautiful and meaningful products\n\nWe strive to follow all UX processes of ideation research and MVP for our product designs to enable output that is both aesthetic and strategic",
      responsibilities: 'Keep the interface beautiful and easy to use',
      jobType: 'Full-Time',
      salary: '$80,000 - $140,000 a year',
      NumOfHires: 2,
    },
  },
  {
    company: 'Google',
    title: 'Cybersecurity Analyst',
    tags: ['Microsoft O365 suite, Azure', 'SSCP'],
    logo: require('../../assets/job/youtube.jpg'),
    hoursAgo: '3 hours ago',
    category: 'Security',
    location: '901 Cherry Ave, San Bruno',
    totalApplied: 13,
    applied: [
      {
        img: require('../../assets/job/image1.png'),
      },
      {
        img: require('../../assets/job/image2.png'),
      },
      {
        img: require('../../assets/job/image3.png'),
      },
      {
        img: require('../../assets/job/image4.png'),
      },
      {
        img: require('../../assets/job/image2.png'),
      },
    ],
    order: {
      message: 'Be in the first',
      countMsg: '13 applicants',
    },
    details: {
      description:
        "We're a team of youthful, intelligent and dedicated creative who have an unrivaled energy and passion for crafting beautiful and meaningful products\n\nWe strive to follow all UX processes of ideation research and MVP for our product designs to enable output that is both aesthetic and strategic",
      responsibilities: 'Keep the interface beautiful and easy to use',
      jobType: 'Full-Time',
      salary: '$90,000 - $150,000 a year',
      NumOfHires: 2,
    },
  },
];

export const categories = [
  {
    name: 'Design',
    count: '1.4k',
  },
  {
    name: 'Engineering',
    count: '3.2k',
  },
  {
    name: 'Medical',
    count: '2.1k',
  },
  {
    name: 'Network',
    count: '649',
  },
  {
    name: 'Security',
    count: '1.6k',
  },
];
