      ...
      <View style={styles.appliedContainer}>
        {people.map((person, i) => (
          <View key={`${i}`} style={styles.appliedCircle}>
            <Image source={person.img} style={styles.personImg} />
          </View>
        ))}
        {isMoreThanLimit && (
          <View
            style={[
              styles.appliedCircle,
            ...
            <Text style={styles.appliedRemainingCount}>
              {Number(totalApplied) - 3}
            </Text>
          </View>
        )}
      </View>
...
const JobCard = React.memo(({item, onJobCardPress}) => {
  ...
  return (
    <TouchableWithoutFeedback onPress={() => onJobCardPress(item)}>
      <View style={[styles.container, cardBackgroundColor(category)]}>
        ...
        <JobStatus {...{order}} />
        <JobCardFooter {...{applied}} {...{hoursAgo}} {...{totalApplied}} />
      </View>
